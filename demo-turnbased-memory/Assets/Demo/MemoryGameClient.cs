...
public class MemoryGameClient : LoadBalancingClient
{
    ...
    {
        Debug.Log(string.Format("OnWebRpcResponse. Code: {0} Content: {1}", response.ResultCode, SupportClass.DictionaryToString(response.Parameters)));
    ...
    public void SaveBoardToProperties()
    {
        ...
        boardProps.Add(GetPlayerPointsPropKey(this.LocalPlayer.ActorNumber), this.MyPoints); // we always only save "our" points. this will not affect the opponent's score.
        ...
        if (board.AreTwoTilesFlipped() && !board.AreTheSameTilesFlipped())
        {
            ...
            if (otherPlayer != null)
            {
                boardProps.Add(PropTurn, otherPlayer.ActorNumber); // used to identify which player's turn the NEXT is. the WebHooks might send a PushMessage to that user.
        ...
        this.OpSetCustomPropertiesOfRoom(boardProps, null, new WebFlags(0) { HttpForward = webForwardToPush });
    }
    ...
    {
        if (player == null || player.ActorNumber < 1)
        {
            return 0;
        }

        string pointsKey = GetPlayerPointsPropKey(player.ActorNumber);
    ...
    public void CreateTurnbasedRoom()
    {
        ...
        this.OpCreateRoom(new EnterRoomParams { RoomName = newRoomName, RoomOptions = roomOptions });

        // as we create a room, we have to trigger a switch to the in-game UI
        this.memoryGui.Visible = false;
        this.board.Visible = false;
        this.memoryGui.CurrentState = MemoryGui.GameState.SwitchToGame;
    }
